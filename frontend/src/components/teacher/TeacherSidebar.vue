...
// Slim icon-only rail vs. the full menu (shared UI store — same as admin).
...
// ---- Bilingual labels (same per-component i18n pattern as the admin sidebar) ----
const content = {
  en: {
  ...
  ar: {
    ...
    brandSub: 'بوابة المعلم'
  }
}
const t = computed(() => content[language.lang])

// ---- Navigation grouped into sections (mirrors the admin sidebar structure) ----
const navGroups = computed(() => [
  {
    label: language.isArabic ? 'الرئيسية' : 'Main',
    items: [
      { to: '/teacher/dashboard', icon: icons.dashboard, label: language.isArabic ? 'الرئيسية' : 'Dashboard' }
    ]
  },
  {
    label: language.isArabic ? 'التدريس' : 'Teaching',
    items: [
      { to: '/teacher/subjects', icon: icons.subjects, label: language.isArabic ? 'موادي' : 'My Subjects' },
      { to: '/teacher/attendance', icon: icons.attendance, label: language.isArabic ? 'الحضور' : 'Attendance' },
      { to: '/teacher/grades', icon: icons.grades, label: language.isArabic ? 'الدرجات' : 'Grades' },
      { to: '/teacher/materials', icon: icons.layers, label: language.isArabic ? 'المواد التعليمية' : 'Learning Materials' }
    ]
  },
  {
    label: language.isArabic ? 'التواصل' : 'Communication',
    items: [
      { to: '/teacher/events', icon: icons.events, label: language.isArabic ? 'الفعاليات والإعلانات' : 'Events & Announcements' }
    ]
  }
])
</script>
...
<style scoped>
.sidebar {
  /* Teacher accent = green (matches the green "Total Teachers" card) */
  --green: #16a34a;
  --green-strong: #12b981;
  ...
  color: #cfe6da;
  /* Deep green shell with a soft emerald glow up top + a faint warm glow at the base */
  background:
    radial-gradient(circle at 22% -8%, rgba(52, 211, 153, 0.22), transparent 44%),
    radial-gradient(circle at 90% 108%, rgba(242, 160, 61, 0.08), transparent 42%),
    linear-gradient(180deg, #0c3125 0%, #082018 100%);
...
/* Active link: highlighted GREEN gradient pill + inline orange accent + lit icon */
.nav-item.router-link-active {
  background: linear-gradient(135deg, var(--green), var(--green-strong));
  color: #fff;
  box-shadow: 0 10px 22px rgba(16, 163, 74, 0.42);
}
