<script setup>
// Reusable gradient page header for admin sub-pages — mirrors the dashboard hero
// (layered glows, dotted texture, orange accent) so every section shares the same
// signature look. Title/subtitle come from the route meta (bilingual), and the
// icon is chosen from the route name, so pages just drop in <AdminPageHeader />.
...
// Map each admin route to its sidebar icon.
const iconByRoute = {
  'admin-users': 'users',
...
<template>
  <header class="page-header" :dir="language.dir">
    ...
    <!-- Optional right-side actions (e.g. a primary button) -->
...
.page-header {
  ...
  gap: 1rem;
  /* Same layered-glow navy gradient as the dashboard hero */
  background:
    ...
    radial-gradient(circle at 82% 100%, rgba(88, 130, 220, 0.42), transparent 52%),
    linear-gradient(135deg, #1b3a7a 0%, #1e295f 55%, #182247 100%);
  box-shadow: 0 14px 32px rgba(24, 34, 71, 0.24);
}
