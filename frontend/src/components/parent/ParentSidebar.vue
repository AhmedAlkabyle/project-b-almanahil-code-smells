...
// Slim icon-only rail vs. the full menu (shared UI store — same as admin/teacher/student).
...
// ---- Bilingual labels (same per-component i18n pattern as the student sidebar) ----
const content = {
  en: {
    brandName: 'Almanahil Libyan School',
    brandSub: 'PARENT PORTAL'
  },
  ar: {
    brandName: 'مدرسة المناهل الليبية',
    brandSub: 'بوابة ولي الأمر'
  }
}
const t = computed(() => content[language.lang])

// ---- Navigation grouped into sections (mirrors the student sidebar structure) ----
const navGroups = computed(() => [
  {
    label: language.isArabic ? 'الرئيسية' : 'Main',
    items: [
      { to: '/parent/dashboard', icon: icons.dashboard, label: language.isArabic ? 'الرئيسية' : 'Dashboard' }
    ]
  },
  {
    label: language.isArabic ? 'المتابعة' : 'Follow-up',
    items: [
      { to: '/parent/attendance', icon: icons.attendance, label: language.isArabic ? 'حضور الطالب' : "Child's Attendance" },
      { to: '/parent/grades', icon: icons.grades, label: language.isArabic ? 'درجات الطالب' : "Child's Grades" },
      { to: '/parent/materials', icon: icons.layers, label: language.isArabic ? 'المواد التعليمية' : 'Learning Materials' }
    ]
  },
  {
    label: language.isArabic ? 'التواصل' : 'Communication',
    items: [
      { to: '/parent/events', icon: icons.events, label: language.isArabic ? 'الفعاليات والإعلانات' : 'Events & Announcements' }
    ]
  }
])
</script>
...
<style scoped>
.sidebar {
  /* Parent accent = slate-gray (matches the neutral "Parents" tone) + shared orange accent */
  --gray: #64748b;
  --gray-strong: #475569;
  ...
  color: #d3dbe6;
  /* Deep slate shell with a soft steel glow up top + a faint warm glow at the base */
  background:
    radial-gradient(circle at 22% -8%, rgba(148, 163, 184, 0.20), transparent 44%),
...
.brand:hover .brand-logo {
  ...
  box-shadow: 0 8px 22px rgba(242, 160, 61, 0.4);
}
...
/* Active link: highlighted SLATE gradient pill + inline orange accent + lit icon */
.nav-item.router-link-active {
  background: linear-gradient(135deg, var(--gray), var(--gray-strong));
  color: #fff;
  box-shadow: 0 10px 22px rgba(51, 65, 85, 0.5);
}
