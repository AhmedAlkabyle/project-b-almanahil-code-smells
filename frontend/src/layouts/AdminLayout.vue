<script setup>
// This is the frame (shell) for every Admin page. It provides the sidebar,
// the top bar (greeting, account menu, language toggle), and the chatbot button
// that wrap around whatever admin page is currently shown. The actual page
...
const roleLabel = computed(() => (language.isArabic ? 'مدير النظام' : 'Administrator'))
...
<template>
  <div class="admin-shell" :dir="language.dir" :lang="language.lang">
    <!-- Decorative floating circles (welcome-hero style), behind everything -->
    <div class="admin-decor" aria-hidden="true">
    ...
    <div class="admin-main">
      <!-- Top bar — a clean utility bar. Leading edge: the sidebar toggle plus a
           greeting/date. Trailing edge: the chatbot shortcut, the account menu
           (profile / logout) and the language toggle. Flips naturally in Arabic (RTL). -->
      <header class="topbar" :dir="language.dir">
        ...
        <div class="topbar-lead">
          ...
          <!-- Greeting + date (shown on every admin page) -->
...
<style scoped>
/* Neutralize the leftover Vite starter constraints on #app (max-width, centered,
   border lines) so the admin layout truly fills the page and connects to the
   sidebar — and the page uses a single default browser scrollbar on the right,
   exactly like the welcome and login pages. */
:global(html),
:global(body),
:global(#app) {
  margin: 0;
...
.admin-shell {
  ...
  display: flex;
  min-height: 100vh;
  /* Soft, pale glow wash over a light gradient — echoes the welcome page's
     section backgrounds so the admin area feels part of the same system. */
  background:
    radial-gradient(circle at 68% 4%, rgba(30, 76, 154, 0.06), transparent 22%),
    radial-gradient(circle at 96% 26%, rgba(242, 160, 61, 0.06), transparent 22%),
    linear-gradient(180deg, #f4f6fb 0%, #eef2f9 100%);
  font-family: 'Segoe UI', system-ui, -apple-system, sans-serif;
}

/* ---- Decorative floating circles (from the welcome hero, tinted for the light
   admin canvas). A fixed, behind-everything layer; the opaque sidebar + white
   content cards sit above, so the circles only peek through the background gaps. */
.admin-decor {
  position: fixed;
  inset: 0;
  z-index: 0;
...
.a-circle.c1 {
  ...
  animation: admin-drift 22s ease-in-out infinite;
}
.a-circle.c2 {
  width: 360px;
  height: 360px;
  ...
  animation: admin-drift 26s ease-in-out infinite reverse;
}
.a-circle.c3 {
  width: 240px;
  height: 240px;
  ...
  animation: admin-drift 30s ease-in-out infinite;
}
...
.a-circle.c4 {
  ...
  border: 1.5px solid rgba(30, 76, 154, 0.12);
  filter: none;
  animation: admin-drift 34s ease-in-out infinite reverse;
}
...
/* Top bar — clean utility bar: chatbot + account + language, aligned to the
   trailing edge (right in EN, left in AR). Each page shows its own gradient header. */
.topbar {
  position: sticky;
  top: 0;
  z-index: 20;
...
.topbar-lang :deep(.lang-toggle) {
  border-color: rgba(30, 76, 154, 0.22);
}

/* Ask-our-chatbot button (placeholder for the future AI assistant) */
.chatbot-btn {
  display: inline-flex;
  align-items: center;
  gap: 0.55rem;
...
.topbar-avatar {
  ...
  box-shadow: 0 0 0 2px #fff, 0 0 0 4px rgba(67, 97, 238, 0.25);
}
.topbar-avatar img,
...
.user-menu {
  ...
  border-radius: 16px;
  box-shadow: 0 20px 44px rgba(15, 36, 68, 0.16);
}
.user-menu-head {
  display: flex;
  align-items: center;
...
@media (max-width: 720px) {
  ...
  /* Keep space-between so the rail toggle stays on the leading edge and the
     account/chatbot actions stay on the trailing edge. */
  .topbar {
    padding: 0.8rem 1rem;
  }
  .admin-content {
