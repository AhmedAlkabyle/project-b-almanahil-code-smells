<script setup>
// This is the frame (shell) for every Student page. It provides the sidebar,
// the top bar (greeting, account menu, language toggle), and the chatbot button
// that wrap around whatever student page is currently shown. The actual page
...
const roleLabel = computed(() => (language.isArabic ? 'طالب' : 'Student'))
...
<template>
  <div class="student-shell" :dir="language.dir" :lang="language.lang">
    <!-- Decorative floating circles (welcome-hero style), behind everything -->
    <div class="student-decor" aria-hidden="true">
    ...
    <div class="student-main">
      <!-- Top bar — same clean utility bar as teacher. Leading edge: the sidebar
      ...
      <header class="topbar" :dir="language.dir">
        ...
        <div class="topbar-lead">
          ...
          <!-- Greeting + date (shown on every student page) -->
...
/* Neutralize the leftover Vite starter constraints on #app so the layout truly
   fills the page and connects to the sidebar (same as the teacher layout). */
:global(html),
:global(body),
:global(#app) {
  margin: 0;
...
.student-shell {
  ...
  display: flex;
  min-height: 100vh;
  /* Soft warm wash over a light cream gradient — the student counterpart to the
     teacher shell (green → orange). */
  background:
    radial-gradient(circle at 68% 4%, rgba(249, 146, 60, 0.06), transparent 22%),
    radial-gradient(circle at 96% 26%, rgba(246, 185, 59, 0.07), transparent 22%),
    linear-gradient(180deg, #fff8f0 0%, #fdf1e4 100%);
  font-family: 'Segoe UI', system-ui, -apple-system, sans-serif;
}

/* ---- Decorative floating circles (welcome-hero style, tinted for the light
   cream student canvas). Fixed, behind-everything layer. */
.student-decor {
  position: fixed;
  inset: 0;
  z-index: 0;
...
.a-circle.c1 {
  ...
  background: rgba(249, 146, 60, 0.1);
  animation: student-drift 22s ease-in-out infinite;
}
.a-circle.c2 {
  width: 360px;
  height: 360px;
  ...
  animation: student-drift 26s ease-in-out infinite reverse;
}
.a-circle.c3 {
  width: 240px;
  height: 240px;
  ...
  animation: student-drift 30s ease-in-out infinite;
}
...
.a-circle.c4 {
  ...
  border: 1.5px solid rgba(239, 138, 41, 0.16);
  filter: none;
  animation: student-drift 34s ease-in-out infinite reverse;
}
@keyframes student-drift {
  0%,
  100% {
...
.topbar {
  ...
  box-shadow: 0 4px 20px rgba(156, 80, 10, 0.05);
}
...
.rail-toggle {
  ...
  color: #b5670f;
  cursor: pointer;
  box-shadow: 0 1px 2px rgba(156, 80, 10, 0.04);
...
.topbar-lang :deep(.lang-toggle) {
  border-color: rgba(239, 138, 41, 0.24);
}

/* Ask-our-chatbot button (identical AI-accent treatment to the admin/teacher side) */
.chatbot-btn {
  display: inline-flex;
  align-items: center;
  gap: 0.55rem;
...
.topbar-user.open {
  border-color: #ecd6bd;
  box-shadow: 0 8px 18px rgba(156, 80, 10, 0.1);
  transform: translateY(-1px);
}
.topbar-avatar {
  width: 40px;
  ...
  font-weight: 800;
  color: #fff;
  background: linear-gradient(135deg, #ef8a29, #f97316);
  box-shadow: 0 0 0 2px #fff, 0 0 0 4px rgba(239, 138, 41, 0.25);
}
.topbar-avatar img,
...
.user-menu {
  ...
  border: 1px solid #f2e4d3;
  border-radius: 16px;
  box-shadow: 0 20px 44px rgba(156, 80, 10, 0.16);
}
.user-menu-head {
  display: flex;
  align-items: center;
