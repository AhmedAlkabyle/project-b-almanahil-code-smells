<script setup>
// This is the frame (shell) for every Parent page. It provides the sidebar,
// the top bar (greeting, account menu, language toggle), and the chatbot button
// that wrap around whatever parent page is currently shown. The actual page
...
const roleLabel = computed(() => (language.isArabic ? 'ولي الأمر' : 'Parent'))
...
// Load the parent's children fresh whenever the parent area mounts (a new login never
// inherits a previous session's children — the store is also reset on logout below).
onMounted(() => parent.loadChildren(true))
...
// Reuse Module 1 logout: clear the child selection + auth, then replace to /login.
...
<template>
  <div class="parent-shell" :dir="language.dir" :lang="language.lang">
    <!-- Decorative floating circles (welcome-hero style), behind everything -->
    <div class="parent-decor" aria-hidden="true">
    ...
    <div class="parent-main">
      <!-- Top bar — same clean utility bar as student. Leading edge: the sidebar
      ...
      <header class="topbar" :dir="language.dir">
        ...
        <div class="topbar-lead">
          ...
          <!-- Greeting + date (shown on every parent page) -->
...
/* Neutralize the leftover Vite starter constraints on #app so the layout truly
   fills the page and connects to the sidebar (same as the student layout). */
:global(html),
:global(body),
:global(#app) {
  margin: 0;
...
.parent-shell {
  ...
  display: flex;
  min-height: 100vh;
  /* Cool light-gray wash — the parent counterpart to the student shell (orange → slate). */
  background:
    radial-gradient(circle at 68% 4%, rgba(100, 116, 139, 0.07), transparent 22%),
    radial-gradient(circle at 96% 26%, rgba(242, 160, 61, 0.05), transparent 22%),
    linear-gradient(180deg, #f8fafc 0%, #f1f5f9 100%);
  font-family: 'Segoe UI', system-ui, -apple-system, sans-serif;
}

/* ---- Decorative floating circles (welcome-hero style, tinted for the light-gray
   parent canvas). Fixed, behind-everything layer. */
.parent-decor {
  position: fixed;
  inset: 0;
  z-index: 0;
...
.a-circle.c1 {
  ...
  animation: parent-drift 22s ease-in-out infinite;
}
.a-circle.c2 {
  width: 360px;
  height: 360px;
  ...
  animation: parent-drift 26s ease-in-out infinite reverse;
}
.a-circle.c3 {
  width: 240px;
  height: 240px;
  ...
  animation: parent-drift 30s ease-in-out infinite;
}
...
.a-circle.c4 {
  ...
  filter: none;
  animation: parent-drift 34s ease-in-out infinite reverse;
}
...
/* Ask-our-chatbot button (identical AI-accent treatment to the other portals) */
.chatbot-btn {
  display: inline-flex;
  align-items: center;
  gap: 0.55rem;
...
.topbar-avatar {
  ...
  box-shadow: 0 0 0 2px #fff, 0 0 0 4px rgba(100, 116, 139, 0.25);
}
.topbar-avatar img,
...
.user-menu {
  ...
  border: 1px solid #e2e8f0;
  border-radius: 16px;
  box-shadow: 0 20px 44px rgba(30, 41, 59, 0.16);
}
.user-menu-head {
  display: flex;
  align-items: center;
