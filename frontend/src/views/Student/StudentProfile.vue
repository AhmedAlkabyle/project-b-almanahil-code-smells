<script setup>
// This is the Student's profile page. It shows the student's account details
// (name, email, role) which are read-only, and it holds the "change password" form.
// To change a password the student types their current password and a new one,
...
const displayName = computed(() => {
  ...
  return `${f} ${l}`.trim() || auth.user?.firstName || 'Student'
})
const email = computed(() => auth.user?.email || '—')
const roleLabel = computed(() => t.value.student)
...
// This function runs when the student clicks "Update password".
    ...
    <div class="cards">
      ...
      <section class="card">
        ...
        <form class="pw-form" @submit.prevent="submitPassword" autocomplete="off">
          <div class="field">
            <label for="sp-current">{{ t.current }}</label>
            <div class="pw-input">
              <input
                id="sp-current"
          ...
          <div class="field">
            <label for="sp-new">{{ t.newPass }}</label>
            <div class="pw-input">
              <input
                id="sp-new"
          ...
          <div class="field">
            <label for="sp-confirm">{{ t.confirm }}</label>
            <div class="pw-input">
              <input
                id="sp-confirm"
                :type="showNew ? 'text' : 'password'"
...
.head-ic {
  ...
  color: var(--orange);
  background: linear-gradient(135deg, rgba(239, 138, 41, 0.14), rgba(246, 185, 59, 0.16));
}
.head-ic svg {
  width: 18px;
  height: 18px;
...
.pw-input input:focus {
  ...
  box-shadow: 0 0 0 3px rgba(239, 138, 41, 0.14);
}
.toggle {
  position: absolute;
...
.btn.primary {
  ...
  box-shadow: 0 8px 18px rgba(239, 138, 41, 0.3);
  align-self: flex-start;
}
.btn.primary:hover:not(:disabled) {
  transform: translateY(-1px);
  box-shadow: 0 12px 24px rgba(239, 138, 41, 0.38);
}
