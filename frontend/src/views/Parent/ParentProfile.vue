<script setup>
// This is the Parent's profile page. It shows the parent's account details
// (name, email, role) which are read-only, and it holds the "change password" form.
// To change a password the parent types their current password and a new one,
// we send them to the server, and we show a success or error message.
...
const displayName = computed(() => {
  ...
  return `${f} ${l}`.trim() || auth.user?.firstName || 'Parent'
})
const email = computed(() => auth.user?.email || '—')
const roleLabel = computed(() => t.value.parent)
...
// on submit and surfaced as friendly messages.
...
// This function runs when the parent clicks "Update password".
    ...
    <div class="cards">
      ...
      <section class="card">
        ...
        <form class="pw-form" @submit.prevent="submitPassword" autocomplete="off">
          <div class="field">
            <label for="pp-current">{{ t.current }}</label>
            <div class="pw-input">
              <input
                id="pp-current"
          ...
          <div class="field">
            <label for="pp-new">{{ t.newPass }}</label>
            <div class="pw-input">
              <input
                id="pp-new"
          ...
          <div class="field">
            <label for="pp-confirm">{{ t.confirm }}</label>
            <div class="pw-input">
              <input
                id="pp-confirm"
                :type="showNew ? 'text' : 'password'"
...
.head-ic {
  ...
  background: linear-gradient(135deg, rgba(100, 116, 139, 0.14), rgba(242, 160, 61, 0.14));
}
.head-ic svg {
  width: 18px;
  height: 18px;
...
.btn.primary {
  ...
  box-shadow: 0 8px 18px rgba(71, 85, 105, 0.3);
  align-self: flex-start;
}
.btn.primary:hover:not(:disabled) {
  transform: translateY(-1px);
  box-shadow: 0 12px 24px rgba(71, 85, 105, 0.38);
}
