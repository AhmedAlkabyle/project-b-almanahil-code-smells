<script setup>
// Parent dashboard home. Slate welcome hero + the SELECTED child's summary cards (class,
// attendance %, average grade — fetched for that child) and a child selector, then a lower
// area with a LIVE "recent announcements" preview (GET /api/events/my) and quick-access
// shortcut cards that link to the parent's real pages. Mirrors the student dashboard,
// themed slate-gray. Bilingual (EN+AR) + RTL.
...
const displayName = computed(() => auth.user?.firstName || 'Parent')

// ---- Bilingual copy (same per-component i18n pattern as the student dashboard) ----
const content = {
  en: {
    welcome: 'Welcome,',
    heroSub: "Follow your child's progress at Almanahil Libyan School.",
    portal: 'Parent portal',
    dash: '—',
    stats: { child: 'Child', myClass: 'Class', attendance: 'Attendance', average: 'Average Grade' },
    ...
    quick: {
      attendance: { t: "Child's Attendance", s: 'Their attendance records' },
      grades: { t: "Child's Grades", s: 'Their grades across subjects' },
      materials: { t: 'Learning Materials', s: 'Resources shared by teachers' },
      events: { t: 'Events & Announcements', s: 'All school news for you' }
    }
  },
  ar: {
    welcome: 'مرحباً،',
    heroSub: 'تابع تقدّم ابنك في مدرسة المناهل الليبية.',
    portal: 'بوابة ولي الأمر',
    dash: '—',
    stats: { child: 'الطالب', myClass: 'الصف', attendance: 'الحضور', average: 'معدل الدرجات' },
    ...
    quick: {
      attendance: { t: 'حضور الطالب', s: 'سجلات حضوره' },
      grades: { t: 'درجات الطالب', s: 'درجاته في جميع المواد' },
      materials: { t: 'المواد التعليمية', s: 'موارد يشاركها المعلمون' },
...
async function loadChildData(childId) {
  ...
  childAttendance.value = attRes.status === 'fulfilled' ? (attRes.value.data ?? []) : []
  childGrades.value = gradeRes.status === 'fulfilled' ? (gradeRes.value.data ?? []) : []
}
watch(() => parent.selectedChildId, loadChildData, { immediate: true })

// ---- Recent announcements (live, parent-scoped events feed) ----
const events = ref([])
...
// ---- Computed stat values (react to the selected child) ----
const childName = computed(() => parent.selectedChild?.studentName || t.value.dash)
const className = computed(() => parent.selectedChild?.className || t.value.dash)
const attendanceRate = computed(() => {
  const total = childAttendance.value.length
  if (!total) return t.value.dash
  const present = childAttendance.value.filter((r) => r.status === 'Present').length
  return `${Math.round((present / total) * 100)}%`
})
const averageGrade = computed(() => {
  const p = childGrades.value.map((r) => (r.maxMark > 0 ? (r.mark / r.maxMark) * 100 : 0))
  if (!p.length) return t.value.dash
  return `${Math.round(p.reduce((a, b) => a + b, 0) / p.length)}%`
})

const statCards = computed(() => [
  { variant: 'slate', label: t.value.stats.child, value: childName.value, icon: icons.user },
  { variant: 'blue', label: t.value.stats.myClass, value: className.value, icon: icons.classes },
...
// ---- Quick-access shortcuts → the parent's real pages (they read the selected child
//      from the shared parent store, so switching child updates each destination). ----
const quickLinks = computed(() => [
  { name: 'parent-attendance', ...t.value.quick.attendance, icon: icons.attendance },
  { name: 'parent-grades', ...t.value.quick.grades, icon: icons.grades },
  { name: 'parent-materials', ...t.value.quick.materials, icon: icons.layers },
  { name: 'parent-events', ...t.value.quick.events, icon: icons.announce }
])
</script>
...
<template>
  <div class="dashboard" :dir="language.dir">
    ...
    <section class="hero">
      ...
      <!-- Selected child's summary cards, right inside the welcome box -->
    ...
    <div class="dash-lower">
      ...
      <section class="block lower-main">
        <div class="block-head with-action">
          ...
          <router-link :to="{ name: 'parent-events' }" class="view-all">
            {{ t.viewAll }}
        ...
        <div class="ann-panel">
          ...
          <ul v-else class="ann-list">
            <li v-for="ev in recentEvents" :key="ev.id">
              <router-link :to="{ name: 'parent-events' }" class="ann-item">
...
.hero {
  ...
  /* Layered radial glows over a deep slate gradient (parent counterpart of the
     orange student hero). */
  background:
    ...
    radial-gradient(circle at 78% 96%, rgba(148, 163, 184, 0.45), transparent 48%),
    linear-gradient(135deg, #475569 0%, #334155 55%, #1e293b 100%);
  box-shadow: 0 18px 40px rgba(30, 41, 59, 0.28);
}
...
.live-dot {
  ...
  border-radius: 50%;
  background: #cbd5e1;
  box-shadow: 0 0 8px rgba(203, 213, 225, 0.9);
}
.hero-title {
...
/* Selected child's summary cards inside the welcome box. */
.hero-stats {
  margin-top: 0.9rem;
  display: grid;
...
/* Short slate→orange accent bar before the section heading */
...
.ann-dot.is-event { background: var(--gray); }
...
.quick-badge {
  ...
  justify-content: center;
  color: var(--gray-strong);
  background: linear-gradient(135deg, rgba(100, 116, 139, 0.14), rgba(242, 160, 61, 0.16));
}
.quick-badge svg {
  width: 20px;
  height: 20px;
