<script setup>
// This is the Admin's profile page. It shows the admin's account details
// (name, email, role) which are read-only, and it holds the "change password" form.
// To change a password the admin types their current password and a new one,
...
const displayName = computed(() => {
  ...
  return `${f} ${l}`.trim() || auth.user?.firstName || 'Admin'
})
const email = computed(() => auth.user?.email || '—')
const roleLabel = computed(() => t.value.admin)
...
// Enabled once all three fields have content; the rules (strength / match /
// different) are checked on submit and surfaced as friendly messages, so the user
// always learns *why* instead of facing a silently-disabled button.
...
// This function runs when the admin clicks "Update password".
    ...
    <div class="cards">
      ...
      <section class="card">
        ...
        <form class="pw-form" @submit.prevent="submitPassword" autocomplete="off">
          <div class="field">
            <label for="pf-current">{{ t.current }}</label>
            <div class="pw-input">
              <input
                id="pf-current"
          ...
          <div class="field">
            <label for="pf-new">{{ t.newPass }}</label>
            <div class="pw-input">
              <input
                id="pf-new"
          ...
          <div class="field">
            <label for="pf-confirm">{{ t.confirm }}</label>
            <div class="pw-input">
              <input
                id="pf-confirm"
                :type="showNew ? 'text' : 'password'"
